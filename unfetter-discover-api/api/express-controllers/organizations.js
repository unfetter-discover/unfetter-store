...
router.get('/request-leadership/:userId/:orgId', (req, res) => {
    ...
    userModel.findById(userId, (err, result) => {
        ...
        if (!matchingOrg) {
            ...
            return res.status(400).json({
                errors: [{
                    status: 400, source: '', title: 'Error', code: '', detail: 'Only standard users may apply for leadership of an organization.'
                }]
            });
        }
...
router.get('/request-membership/:userId/:orgId', (req, res) => {
    ...
    userModel.findById(userId, (err, result) => {
        ...
        if (matchingOrg) {
            return res.status(500).json({
                errors: [{
                    status: 500, source: '', title: 'Error', code: '', detail: 'User is already affiliated with this organization'
                }]
            });
        }
